...
import psycopg2
# helper code for formatting output with python: https://pynative.com/python-postgresql-select-data-from-table/
# helper method for dividing columns without getting 0: https://stackoverflow.com/questions/3996779/how-to-divide-two-columns/3996808
# helper codes for cast, to_char, rount functions where used from multiple online sources like: postgresql.org and w3schools.com
try:
...
#check view number 1 to understand the following:
    authors_query = """ SELECT authcles.name, count(*) as views
...
#check views number 2, 3, 4, 5 to understand the following:
    errors_query = """ select to_char(date, 'Mon DD, YYYY'), round((CAST(bad_c AS DECIMAL) / all_c)*100, 1) from clicksbg_h;
    """
